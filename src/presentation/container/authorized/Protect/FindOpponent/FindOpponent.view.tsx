...
const _FindOpponentProtected: React.FC = () => {
    ...
    const handlePress = async () => {
      try {
        const playerName = await joinMatchQueueWithUserData(userId!, 'game3'); 
        setName(playerName); 
  
        navigation.navigate('WaitingLuckeyMoney', { name: playerName , game: 'game3'});
      } catch (err) {
    ...
    return (
        <ImageBackground source={BACKGROUND_FINDOPPONENTS_SHOOTTHEGREW} style={styles.background} resizeMode="stretch">
            ...
            <ImageBackground source={LABEL_FINDOPPONENTS_SHOOTTHEGREW} style={styles.label} resizeMode='cover'>
                ...
                <Text style={styles.text2}> Chạm nhanh tay để ngăn chặn dòng nước tấn công công trình của bạn! Càng chạm nhiều, điểm càng cao! Hãy chiến đấu để trở thành người hùng chống thấm giỏi nhất và đánh bại đối thủ!
                </Text>
...
export const FindOpponentProtected = React.memo(_FindOpponentProtected);
